...
from Classes import *
import FFEnergy, Visualize, WaterModelAssign
...
class MainPage(object):
    ...
    def __init__(self, master, info='GUI for FFEnergy program'):
        ...
        self.main_label.grid(row=0, columnspan=12)

        self.crd_label = Label(master, text='Coordinates file:')
        self.crd_label.grid(row=1, sticky=W)
        self.crd_entry = Entry(master)
        self.crd_entry.grid(row=1, column=2, columnspan=8)
        self.crd_entry.insert(0,'.crd')
        self.crd_button = Button(master, text='Choose', command=self.choose_crd_file)
        self.crd_button.grid(row=1, column=11)

        self.ff1_label = Label(master, text='Force field file 1 (general, mostly for the host):')
        self.ff1_label.grid(row=2, sticky=W)
        self.ff1_entry = Entry(master)
        self.ff1_entry.grid(row=2, column=2, columnspan=8)
        self.ff1_entry.insert(0,'.ff')
        self.ff1_button = Button(master, text='Choose', command=self.choose_ff1_file)
        self.ff1_button.grid(row=2, column=11)

        self.ff2_label = Label(master, text='Force field file 2 (specific, mostly for the guest):')
        self.ff2_label.grid(row=3, sticky=W)
        self.ff2_entry = Entry(master)
        self.ff2_entry.grid(row=3, column=2, columnspan=8)
        self.ff2_entry.insert(0,'.ff')
        self.ff2_button = Button(master, text='Choose', command=self.choose_ff2_file)
        self.ff2_button.grid(row=3, column=11)

        self.load_button = Button(master, text='Load Files', command=self.load_files)
        self.load_button.grid(row=4, sticky=W)
        self.view_button = Button(master, text='Visualize Structure', command=self.view_crd)
        self.view_button.grid(row=4, column=2)

        self.guest_label = Label(master, text='Guest atoms (1-start, separate with comma):')
        self.guest_label.grid(row=5, sticky=W)
        self.guest_entry = Entry(master)
        self.guest_entry.grid(row=5, column=2, columnspan=8)
        self.guest_entry.insert(0,'')
        self.guest_button = Button(master, text='Set Guests', command=self.set_guests)
        self.guest_button.grid(row=5, column=11)
